...
function dump(word) {
  var t = V4.traitsFor(word), out = [];
  for (var p = 0; p < 4; p++) for (var b = 0; b < 2; b++)
    out.push(V4.renderFull(t, p, !!b).map(function (v) { return v || '.'; }).join(''));
...
var cold = require('child_process').execSync(
  'node -e "var V=require(\'./wordpet4.js\'),c=require(\'crypto\'),o=[];var t=V.traitsFor(\'emino\');' +
...
var src = fs.readFileSync('./wordpet4.js', 'utf8');
...
var KEYS = ['colour', 'species', 'hat', 'worn', 'eyes', 'shoes', 'item1', 'item2', 'companion', 'scene'];
var N = 120000, checked = 0, seenSlots = {};
for (var i = 0; i < N; i++) {
  var t = V4.traitsFor('rt' + i);

  var fe = V4.parseEmoji(t.emojiLine);
  assert.ok(fe, 'emoji line did not parse: ' + t.emojiLine);
  var fp = V4.parsePhrase(t.phrase);
  ...
  [['hat', 'hatColour'], ['worn', 'wornColour'], ['shoes', 'shoeColour']].forEach(function (p) {
    var want = t[p[0]] === 0 ? null : t[p[1]];
    assert.strictEqual(fe[p[1]], want); assert.strictEqual(fp[p[1]], want);
  });

  /* the drawing reads exactly these traits */
  var v2 = V4.toV2(t);
  assert.strictEqual(v2.species, V4.SPECIES[t.species].v2);
  assert.strictEqual(v2.hat, V4.HATS[t.hat].v2);
  assert.strictEqual(v2.eyes, V4.EYES[t.eyes].v2);
  assert.strictEqual(v2.shoes, V4.SHOES[t.shoes].v2);
  assert.strictEqual(v2.pattern, 0);
  assert.strictEqual(v2.item, 0, 'v4 draws the held items itself, so v2 must draw none');
  ...
  assert.deepStrictEqual(uniq, V4.SLOTS, 'sentence slot order must match the emoji groups');
  assert.deepStrictEqual(t.emojiGroups.map(function (g) { return g.slot; }), V4.SLOTS);
...
for (i = 0; i < 80000; i++) {
  var c = V4.traitsFor('cpl' + i), sp = V2.SPECIES[V4.SPECIES[c.species].v2];
  if (sp.limb === 1 || sp.sits) assert.strictEqual(c.shoes, 0, sp.n + ' has no feet');
  if (sp.limb === 2) { assert.strictEqual(c.item1, 0, sp.n + ' has wings, not hands'); assert.strictEqual(c.item2, 0); }
...
var traitEmoji = {};
[V4.COLOURS, V4.SPECIES, V4.HATS, V4.EYES, V4.SHOES, V4.ITEMS, V4.WORN, V4.COMPANIONS, V4.SCENES]
  .forEach(function (l) { l.forEach(function (o) { traitEmoji[o.e] = 1; }); });
for (i = 0; i < 40000; i++) {
  var ts = V4.traitsFor('seal' + i);
...
console.log('5. seal comes from a pool of ' + V4.SEAL_POOL.length + ' emoji naming no trait; '
  + varied.toLocaleString('en-US') + ' repeated phrases carried a different seal');

/* ---------- 6. the scene stays behind the pet ---------- */
for (i = 0; i < 2000; i++) {
  var st = V4.traitsFor('scene' + i);
  var fg = V4.render(st, 0, false), bg = V4.renderScene(st), full = V4.renderFull(st, 0, false);
...
for (i = 0; i < R; i++) V4.traitsFor('rate' + i);
...
[['colour + species + scene', V4.COLOURS.length * V4.SPECIES.length * V4.SCENES.length],
...
for (i = 0; i < M; i++) seenId.add(V4.traitsFor('u' + i + '-' + (i * 2654435761 % 1000003)).phrase);
