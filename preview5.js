/* preview5.js - v5 contact sheet.  node preview5.js [out.png] [names...] */
...
var out = process.argv[2] || 'preview5.png';
var mode = process.argv[3];
var cells = [];
if (mode === 'species') {
  V5.SPECIES.forEach(function (sp, i) {
    var t = V5.traitsFor('sp-' + sp.w);
    t.species = i; t.hat = 0; t.clothes = 0; t.item1 = 0; t.item2 = 0;
    t.companion = 0; t.scene = 0; t.eyes = 0;
    t = Object.assign(V5.traitsFor('sp-' + sp.w), { species: i, hat: 0, clothes: 0,
        item1: 0, item2: 0, companion: 0, scene: 0, eyes: 0, shoes: 0 });
    cells.push({ buf: V5.renderFull(t, 0, false), label: sp.w });
  });
} else {
  ...
  names.forEach(function (n) { cells.push({ buf: V5.renderFull(V5.traitsFor(n), 0, false), label: n }); });
}
var SC = 3, COLS = 8, CELL = V5.W * SC, ROWS = Math.ceil(cells.length / COLS);
...
cells.forEach(function (cell, k) {
  ...
  for (var y = 0; y < V5.H; y++) for (var x = 0; x < V5.W; x++) {
    var v = cell.buf[y*V5.W+x], px = v ? rgbOf(v) : bg;
